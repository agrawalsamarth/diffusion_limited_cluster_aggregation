...
df_vs_seed     = pd.DataFrame(columns=['N_s','d_f'])
r_min          = 5
rog_prefactor  = 0.8
N              = 2048
...
for N_s in range(1,21):
    ...
    pair_filename=seed_folder+'pair_correlation_'+str(N_s)+'_0.csv'
    ...
    model.fit(x,y)
    df_vs_seed.loc[len(df_vs_seed)] = [N_s,model.coef_[0]]
    
plt.figure(dpi=300)
plt.scatter(df_vs_seed['N_s'], df_vs_seed['d_f'])
plt.xlabel(r'$N_s$')
plt.ylabel(r'$d_f$')
plt.text(15,1.6,'N=2048')
plt.savefig('./plots/df_vs_ns.pdf', format='pdf')
plt.clf()

y_preds=model.predict(x)
plt.scatter(x,y_preds,c='red')
plt.plot(x,y)
plt.savefig('./plots/fit.pdf', format='pdf')