...
class dlca_lattice_3d
{
  public:
    ...
    static const int N  = cell3d::N;
    static const int D  = cell3d::D;
    static const int L  = cell3d::L;
    const double s_fraction = 1.0;
    const int N_s = (int)(N * s_fraction);
    //const int N_s = 1;


    dlca_lattice_3d(int seed)
    {
        D_eff  = (2*D);
    ...
    initArrays()
    {
        ...
        for (int i = 1; i < N; i++){
            ...
            while (flag == 0){
                ...
                curr_index   = grid.occupied(x_,y_,z_);
                left_index   = grid.occupied(left,y_,z_);
                right_index  = grid.occupied(right,y_,z_);
                down_index   = grid.occupied(x_,down,z_);
                up_index     = grid.occupied(x_,up,z_);
                behind_index = grid.occupied(x_,y_,behind);
                ahead_index  = grid.occupied(x_,y_,ahead);

                if ((curr_index == N) && (left_index == N) && (right_index == N) && (down_index == N) && (up_index == N) &&
                    (behind_index == N) && (ahead_index == N)) {
                    pos(i, 0) = x_;
                    pos(i, 1) = y_;
                    pos(i, 2) = z_;

                    grid.occupied(x_,y_,z_) = i;
                    flag = 1;
    ...
    moveCluster()
    {
        ...
        temp_rand = 3. * dis(generator);
        axis_rand = (0 * (temp_rand < 1.0)) + (1 * (temp_rand >= 1.0) * (temp_rand < 2.0)) + (2 * (temp_rand >= 2.0));
        ...
        //std::cout<<"axis_rand = "<<axis_rand<<"\t sign_rand = "<<sign_rand<<std::endl;
        ...
        //for (int axis = 0; axis < 6; axis++){
            for (int i = 0; i < N; i++)
            {
                if (clusterNumber_[i] == index){
                    ...
                    checkForAggregations(x_, y_, z_, i);
                }
            ...
            if (j != N){
                ...
                if (clusterIndex_1 != clusterIndex_2){
                    ...
                    if ((seed_1 == 1) && (seed_2 == 1)){
                        ...
                        /*if ((i == 3) && (j == 20)){
                            std::cout<<"BINDING counter = "<<counter<<"\t 1 = "<<pos(i,0)<<" "<<pos(i,1)<<" "<<pos(i,2)<<"\t 2 = "<<
                            pos(j,0)<<" "<<pos(j,1)<<" "<<pos(j,2)<<std::endl;
                        }
    ...
    void
    checkForAggregations(const int x, const int y, const int z, const int i) {
            ...
            neighbour_index = grid.occupied(left, y, z);
            checkForNeighbour(i, neighbour_index);

            seed_1         = seed_[i];
            clusterIndex_1 = clusterNumber_[i];

            neighbour_index = grid.occupied(right, y, z);
            checkForNeighbour(i, neighbour_index);

            seed_1         = seed_[i];
            clusterIndex_1 = clusterNumber_[i];

            neighbour_index = grid.occupied(x, down, z);
            checkForNeighbour(i, neighbour_index);

            seed_1         = seed_[i];
            clusterIndex_1 = clusterNumber_[i];

            neighbour_index = grid.occupied(x, up, z);
            checkForNeighbour(i, neighbour_index);

            seed_1         = seed_[i];
            clusterIndex_1 = clusterNumber_[i];

            neighbour_index = grid.occupied(x, y, behind);
            checkForNeighbour(i, neighbour_index);

            seed_1         = seed_[i];
            clusterIndex_1 = clusterNumber_[i];

            neighbour_index = grid.occupied(x, y, ahead);
            checkForNeighbour(i, neighbour_index);
