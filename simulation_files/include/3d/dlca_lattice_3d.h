...
#ifndef DLCA_LATTICE_3D_H
#define DLCA_LATTICE_3D_H
...
class dlca_lattice_3d
{
  public:
    ...
    initArrays()
    {
        ...
        for (int i = 1; i < N; i++){
            ...
            while (flag == 0){
                ...
                for (int axis = 0; axis < D; axis++)
                    location[axis] = (int)(L * dis(generator)); 
    ...
    moveCluster()
    {
        ...
        for (int i = 0; i < N; i++){
            if (clusterNumber_[i] == index){
                ...
                if ((seed_[index] == 1) || ((seed_[index] == 0) && (curr_index == N))){
        ...
        //for (int axis = 0; axis < 6; axis++){
            for (int i = 0; i < N; i++)
            {
                if (clusterNumber_[i] == index){
                    ...
                    checkForAggregations(location, i);
                }
    ...
    bindAggregates(const int index_1, const int index_2)
    {
        ...
        if ((seed_[index_1] == 0) || (seed_[index_1] == 0))
            totalWalkers_--;
        else
            totalClusters_--;
    ...
    void
    checkForAggregations(int *location) {

        index_1        = grid.occupied(location);
        clusterIndex_1 = clusterNumber_[index_1];
        ...
        for (int axis = 0; axis < D; axis++){
            for (int dir = 0; dir < 2; dir++){
                ...
                if (index_2 != N){
                    if (seed_[index_2] == 0)
                        checkForAggregations(nb_location);
                }
